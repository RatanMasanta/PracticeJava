...
public class DeterminentOfAMatrix {
    ...
    static int determinantOfMatrix(int matrix[][], int n)
    {
        ...
        for(int i=0;i<n;i++){
            ...
            ans += matrix[0][i]
                    * determinantOfMatrix(second,n-1)*((i&1)==1?-1:1); // if 'i' is odd then multiply it by -1 and if is even then with +1
        }
        return ans;
    }

    public static void main(String[] args) {
        int[][] matrix = {{1, 0, 2, -1}, {3, 0, 0, 5}, {2, 1, 4, -3}, {1, 0, 5, 0}};
        int noOfRows = 4;
        System.out.println(DeterminentOfAMatrix.determinantOfMatrix(matrix,noOfRows));
    }
}
